...
Search for MINT.md files using Glob with `**/MINT.md` scoped to the current
working directory and its parent. Do NOT search all of `~/`.

**If 0 found:** "No MINT.md found. Run /mint-system first to create your design system."

**If 1 found:** Use automatically. "Found MINT.md at [path]. Using this."
...
**Step 6d: Update MINT.md** with the new tokens. Append to the Component Tokens section.
...
After all 3 DNA components are approved, write patterns to MINT.md (append to
the Component Tokens section):
...
Append component library reference:
